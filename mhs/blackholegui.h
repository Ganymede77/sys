#ifndef BLACKHOLEGUI_H__
#define BLACKHOLEGUI_H__

#include "commons.h"

//  \brief This class represents a configured MH of the type Black Hole.
/*
  This is a subclass of MhSets and represents a configured Black Hole.
  It has all the members and configuration variables that a Black Hole
  need to work with a problem.
*/
//BLACKHOLE

class BlackHoleGUI //: public Metaheuristics
{
public:
    BlackHoleGUI();
    virtual ~BlackHoleGUI();
    BlackHoleGUI(const BlackHoleGUI& other);
    BlackHoleGUI& operator=(const BlackHoleGUI& other);
    ...
    void LoadProjectMH(QMap <QString, QVariant> * ruleToStop, QMap <QString, QVariant> * heuristicPersistence, QMap <QString, QVariant> * heuristicParameters, QMap<int, int> * currentMhPositions, QFile * f, int * i, int VelocityQtd);
    void SetSpecificMhSettingsUi(QFrame * frm_rule_to_stop_heupersis, QWidgetList * mh_def_widgets, QGridLayout * rule_to_stop_grid, QGridLayout * frm_rule_to_stop_heupersis_grid, QFrame * frm_heuristic_parameter, QGridLayout * heuristic_parameter_grid, QLabel * lbl_check_equations);
...
public:
    ...
    void SetVelQtd(int val) { velQtd = val; }
    int  GetVelQtd(void) { return velQtd; }
...
private:
    ...
    QComboBox *cmbBx_sch_radius_calculus;
    WidgetListInput *spnBx_heupar_swarm_size;
    QCheckBox *bx_heupar_generate_dead_stars;

public:
    QComboBox &GetCmbBxSchRadiusCalculus() const {return *cmbBx_sch_radius_calculus;}
    WidgetListInput &GetSpnbxHeuparSwarmSize() const {return *spnBx_heupar_swarm_size;}
    QCheckBox &GetBxHeuparGenerateDeadStars() const {return *bx_heupar_generate_dead_stars;}

    void SetCmbBxSchRadiusCalculus (QString val) {cmbBx_sch_radius_calculus->setCurrentText(val);}
    void SetSpnBxHeuparSwarmSize (QVariantList val) {spnBx_heupar_swarm_size->SetValues(val);}
    void SetBxHeuparGenerateDeadStars (bool val) {bx_heupar_generate_dead_stars->setChecked(val);}
