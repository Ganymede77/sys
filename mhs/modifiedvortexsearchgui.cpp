#include "modifiedvortexsearchgui.h"

//MODIFIEDVORTEXSEARCHGUI
ModifiedVortexSearchGUI::ModifiedVortexSearchGUI()
{
    //ctor
    ...
    cmbBx_neigh_search_strat_type->setCurrentText("NEIGHBOUR_VORTEX");
...
ModifiedVortexSearchGUI::ModifiedVortexSearchGUI(const ModifiedVortexSearchGUI& other)
{
    // copy
...
ModifiedVortexSearchGUI&  ModifiedVortexSearchGUI::operator=(const ModifiedVortexSearchGUI& other)
{
    // = operator
...
//SÓ É CHAMADA QUANDO O PROJETO É CARREGADO!
void ModifiedVortexSearchGUI::SetMhParameters(QMap<QString, QVariant> ruleToStop, QMap<QString, QVariant> heuristicPersistence, QMap<QString, QVariant> heuristicParameters){
    //rule to stop
...
long ModifiedVortexSearchGUI::GetParamCount()
{
    long buffer = 1;
...
QString ModifiedVortexSearchGUI::RefreshInfo()
{
    ...
    updated_info += " - Number of Vortices (Centers): " + GenerateWidgetListInputInfo(this->GetSpnbxBestSize()) + "\n";
...
void ModifiedVortexSearchGUI::writeInGeneralConfigurationXML(QString * selectedMh, QXmlStreamWriter * xmlWriterThat, int MhParam)
{
    ...
    xmlWriterThat->writeAttribute("ID", "MODIFIEDVORTEXSEARCH");
    ...
    xmlWriterThat->writeTextElement("DecayFunction", GetCmbBxDecayFunction().currentText());
    ...
    xmlWriterThat->writeTextElement("GammaDecreaseIntensity", parValue.at(vParamIndices[2]).toString());
    ...
    if(GetCmbBxNeighSearchStratType().currentText() == "NEIGHBOUR_VORTEX")
    {
        xmlWriterThat->writeStartElement("SearchingWay");
        xmlWriterThat->writeAttribute("Type", "NEIGHBOUR_VORTEX");
        xmlWriterThat->writeEndElement();
    }
    if(GetCmbBxNeighSearchStratType().currentText() == "NEIGHBOUR_SINECOSINE")
    {
        xmlWriterThat->writeStartElement("SearchingWay");
...
void ModifiedVortexSearchGUI::writeProjectSaveFile(QXmlStreamWriter * xmlWriter)
{
    xmlWriter->writeStartElement("HeuristicType"); //starts the HeuristicType tag
    xmlWriter->writeAttribute("Activated", "1");
    xmlWriter->writeAttribute("ID", "MODIFIEDVORTEXSEARCH");
    ...
    for(int i = 0; i < GetSpnbxBestSize().GetValues().size(); ++i)
        xmlWriter->writeAttribute("BestSize_MVS" + QString::number(i), GetSpnbxBestSize().GetValues().at(i).toString());

    for(int i = 0; i < GetSpnbxHeuparNeighSize().GetValues().size(); ++i)
        xmlWriter->writeAttribute("NeighbourhoodSize_MVS" + QString::number(i), GetSpnbxHeuparNeighSize().GetValues().at(i).toString());

    for(int i = 0; i < GetDSpnBxGammaDecreaseIntensity().GetValues().size(); ++i)
        xmlWriter->writeAttribute("GammaDecreaseIntensity_MVS" + QString::number(i), GetDSpnBxGammaDecreaseIntensity().GetValues().at(i).toString());

    xmlWriter->writeAttribute("DecayFunction_MVS", GetCmbBxDecayFunction().currentText());
    xmlWriter->writeStartElement("NeighbourSearchingtrategy");
    xmlWriter->writeAttribute("Type_MVS", GetCmbBxNeighSearchStratType().currentText());
...
void ModifiedVortexSearchGUI::LoadProjectMH(QMap <QString, QVariant> * ruleToStop, QMap <QString, QVariant> * heuristicPersistence, QMap <QString, QVariant> * heuristicParameters, QMap<int, int> * currentMhPositions, QFile * f, int * i)
{
    //rule to stop
    ...
    //best size
    heuristicParameters->unite(LoadMultipleMhAttFromFile(f, "SpnBxBestSize", "HeuristicParameters", "BestSize_MVS").value(currentMhPositions->value(*i)));

    //heuristic parameters
    heuristicParameters->unite(LoadMultipleMhAttFromFile(f, "SpnBxHeuparNeighSize", "HeuristicParameters", "NeighbourhoodSize_MVS").value(currentMhPositions->value(*i)));
    heuristicParameters->unite(LoadMultipleMhAttFromFile(f, "DSpnBxGammaDecreaseIntensity", "HeuristicParameters", "GammaDecreaseIntensity_MVS").value(currentMhPositions->value(*i)));
    heuristicParameters->insert("CmbBxDecayFunction", GetXmlAttribute(f, "HeuristicParameters", "DecayFunction_MVS").at(currentMhPositions->value(*i)));

    heuristicParameters->insert("CmbBxNeighSearchStratType", GetXmlAttribute(f, "NeighbourSearchingtrategy", "Type_MVS").at(currentMhPositions->value(*i)));

}

void ModifiedVortexSearchGUI::SetSpecificMhSettingsUi(QFrame * frm_rule_to_stop_heupersis, QWidgetList * mh_def_widgets, QGridLayout * rule_to_stop_grid, QGridLayout * frm_rule_to_stop_heupersis_grid, QFrame * frm_heuristic_parameter, QGridLayout * heuristic_parameter_grid)
{
    // ///////////////////"rule to stop" box////////////////////////
    ...
    // /////////////Best Size
    QGroupBox *frm_best_size = new QGroupBox("Number of Vortices (Centers)", frm_rule_to_stop_heupersis);
    ...
    QLabel *lbl_heupar_decay_function = new QLabel("Decay Function");
