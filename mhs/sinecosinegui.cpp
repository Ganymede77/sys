#include "sinecosinegui.h"

//SINECOSINEGUI
SineCosineAlgorithmGUI::SineCosineAlgorithmGUI()
{
    //ctor
    ...
    dSpnBx_max_radius = new WidgetListInput(QVariant::Double);
    ...
    cmbBx_neigh_search_strat_type->addItem("NEIGHBOUR_VORTEX");
    cmbBx_neigh_search_strat_type->addItem("NEIGHBOUR_SINECOSINE");
    cmbBx_neigh_search_strat_type->setCurrentText("NEIGHBOUR_SINECOSINE");
    spnBx_heupar_neigh_size->SetValue(30);
    dSpnBx_max_radius->SetValue("2.0");
...
SineCosineAlgorithmGUI::SineCosineAlgorithmGUI(const SineCosineAlgorithmGUI& other)
{
    // copy
    ...
    dSpnBx_max_radius = other.dSpnBx_max_radius;
    spnBx_heupar_neigh_size = other.spnBx_heupar_neigh_size;
...
SineCosineAlgorithmGUI&  SineCosineAlgorithmGUI::operator=(const SineCosineAlgorithmGUI& other)
{
    // = operator
...
void SineCosineAlgorithmGUI::SetMhParameters(QMap<QString, QVariant> ruleToStop, QMap<QString, QVariant> heuristicPersistence, QMap<QString, QVariant> heuristicParameters){
    ...
    this->SetCmbBxMaxRadius(GetMultipleAttFromMap(heuristicParameters, "dSpnBxMaxRadius"));
    this->SetCmbBxNeighSearchStratType(heuristicParameters.value("CmbBxNeighSearchStratType").toString());

}

long SineCosineAlgorithmGUI::GetParamCount() //PEGA O NÚMERO DE DIFERENTES PARÂMETROS DAS CAIXAS QUE PERMITEM MÚLTIPLAS INSERÇÕES!
{
    long buffer = 1;
    buffer *= this->GetSpnbxHeuparNeighSize().GetValues().size();
    buffer *= this->GetSpnBxMaxRadius().GetValues().size();
    buffer *= this->GetSpnbxBestSize().GetValues().size();

    return buffer;
}

QString SineCosineAlgorithmGUI::RefreshInfo()
{
    ...
    updated_info += " - Maximum Search Radius: " + GenerateWidgetListInputInfo(this->GetSpnBxMaxRadius()) + "\n";
    updated_info += " - Population Size: " + GenerateWidgetListInputInfo(this->GetSpnbxHeuparNeighSize()) + "\n";
    updated_info += " - Neighbour Searching Strategy:\n";
    updated_info += "  Type: " + this->GetCmbBxNeighSearchStratType().currentText() + "\n";
    if(this->GetCmbBxNeighSearchStratType().currentText() == "NEIGHBOUR_VORTEX")
    {
        updated_info += "  Searching Way:\n";
        updated_info += "    Type: " + this->GetCmbBxNeighSearchStratType().currentText() + "\n";
    }
    if(this->GetCmbBxNeighSearchStratType().currentText() == "NEIGHBOUR_SINECOSINE")
    {
        updated_info += "  Searching Way:\n";
        updated_info += "    Type: " + this->GetCmbBxNeighSearchStratType().currentText() + "\n";
    }
...
void SineCosineAlgorithmGUI::writeInGeneralConfigurationXML(QString * selectedMh, QXmlStreamWriter * xmlWriterThat, int MhParam)
{
    if(vParamIndicesFirstTime)
    {
        ...
        vP.push_back(GetSpnBxMaxRadius().GetValues().size());
    ...
    xmlWriterThat->writeAttribute("ID", "SINECOSINE");
    ...
    xmlWriterThat->writeTextElement("PopulationSize", parValue.at(vParamIndices[0]).toString());

    parValue = GetSpnBxMaxRadius().GetValues();
    xmlWriterThat->writeTextElement("a", parValue.at(vParamIndices[1]).toString());
    ...
    if(GetCmbBxNeighSearchStratType().currentText() == "NEIGHBOUR_HAMMING")
    {
        ...
        xmlWriterThat->writeAttribute("HammingDistance", "0");
        xmlWriterThat->writeEndElement();
    }
    ...
    else if(GetCmbBxNeighSearchStratType().currentText() == "NEIGHBOUR_RANDOMNESSBALL")
    {
        ...
        xmlWriterThat->writeAttribute("RadiusRate", "0");
        xmlWriterThat->writeEndElement();
    }
...
void SineCosineAlgorithmGUI::writeProjectSaveFile(QXmlStreamWriter * xmlWriter)
{
    xmlWriter->writeStartElement("HeuristicType"); //starts the HeuristicType tag
    xmlWriter->writeAttribute("Activated", "1");
    xmlWriter->writeAttribute("ID", "SINECOSINE");
    ...
    for(int i = 0; i < GetSpnbxBestSize().GetValues().size(); ++i)
        xmlWriter->writeAttribute("BestSize_SCA" + QString::number(i), GetSpnbxBestSize().GetValues().at(i).toString());

    for(int i = 0; i < GetSpnbxHeuparNeighSize().GetValues().size(); ++i)
        xmlWriter->writeAttribute("PopulationSize_SCA" + QString::number(i), GetSpnbxHeuparNeighSize().GetValues().at(i).toString());

    for(int i = 0; i < GetSpnBxMaxRadius().GetValues().size(); ++i)
        xmlWriter->writeAttribute("a_SCA" + QString::number(i), GetSpnBxMaxRadius().GetValues().at(i).toString());

    xmlWriter->writeStartElement("NeighbourSearchingtrategy");
    xmlWriter->writeAttribute("Type_SCA", GetCmbBxNeighSearchStratType().currentText());
...
void SineCosineAlgorithmGUI::LoadProjectMH(QMap <QString, QVariant> * ruleToStop, QMap <QString, QVariant> * heuristicPersistence, QMap <QString, QVariant> * heuristicParameters, QMap<int, int> * currentMhPositions, QFile * f, int * i)
{
    //rule to stop
    ...
    //best size
    heuristicParameters->unite(LoadMultipleMhAttFromFile(f, "SpnBxBestSize", "HeuristicParameters", "BestSize_SCA").value(currentMhPositions->value(*i)));

    //heuristic parameters
    heuristicParameters->unite(LoadMultipleMhAttFromFile(f, "SpnBxHeuparNeighSize", "HeuristicParameters", "PopulationSize_SCA").value(currentMhPositions->value(*i)));
    heuristicParameters->unite(LoadMultipleMhAttFromFile(f, "dSpnBxMaxRadius", "HeuristicParameters", "a_SCA").value(currentMhPositions->value(*i)));

    heuristicParameters->insert("CmbBxNeighSearchStratType", GetXmlAttribute(f, "NeighbourSearchingtrategy", "Type_SCA").at(currentMhPositions->value(*i)));
}

void SineCosineAlgorithmGUI::SetSpecificMhSettingsUi(QFrame * frm_rule_to_stop_heupersis, QWidgetList * mh_def_widgets, QGridLayout * rule_to_stop_grid, QGridLayout * frm_rule_to_stop_heupersis_grid, QFrame * frm_heuristic_parameter, QGridLayout * heuristic_parameter_grid)
{
    // ///////////////////"rule to stop" box////////////////////////
    ...
    QLabel *lbl_heupar_max_radius = new QLabel("Maximum search radius (\"a\")");
    QLabel *lbl_heupar_neigh_size = new QLabel("Population Size");
    ...
    WidgetListInput *dSpnBx_max_radius = &GetSpnBxMaxRadius();
    ...
    QGroupBox *group_neighbourhood = new QGroupBox("Population");
    ...
    QGroupBox *group_decrease = new QGroupBox("Max Radius");
    QGridLayout *lay_group_decrease = new QGridLayout();
    //CURRENTLY DISABLED
    lay_group_decrease->addWidget(lbl_heupar_max_radius, 1, 0, Qt::AlignLeft);
    lay_group_decrease->addWidget(dSpnBx_max_radius, 1, 1, Qt::AlignLeft);
